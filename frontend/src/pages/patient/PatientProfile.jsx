...
const PatientProfile = () => {
  ...
  const handleUpdateReschedule = async () => {
    ...
    try {
      const selectedSlot = rescheduledata.time;
      if (!selectedSlot) {
        toast.error("Invalid time slot selected.");
        return;
      }
      const [startTime, endTime] = selectedSlot.split(/\s*-\s*/);
  ...
  const changeLocation = async () => {
    if (!navigator.onLine) {
      toast.error(
        "You're offline. We may not be able to update your current location accurately."
      );
      return;
    }
  ...
  const HandleavailableDays = async (id) => {
    ...
    try {
      ...
      if (res.status == 200) {
        ...
        console.log("No data in available days");
      }
    } catch (error) {
      console.log("No data in available days in catch block", error);
    }
  };
